...
public class DesignspirationScrape extends App {
	Document doc;
	...
	private void readHTML() {
		try {
			doc=Jsoup.connect("http://designspiration.net/mariuswatz/").get();
			String title = doc.title();
			System.out.println("title "+title);
			
			images=new ArrayList<String>();
			
			Elements img = doc.select("a.post"); // a with href
			for(Element el : img) {
				String s=el.html();
			...
			Elements page=doc.select("span.page a"); // a with href
			for(Element el : page) {
	...
	static public void main(String args[]) {
		...
		PApplet.main(new String[] { "scrape.DesignspirationScrape" });
